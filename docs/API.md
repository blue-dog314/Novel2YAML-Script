...
后端内置一个本地「作者工作台」单页界面(无构建 Vue 3 + 原生 fetch），由 FastAPI 通过 `StaticFiles` 同源挂载在 `/app`。
...
工作台只调用本文件描述的现有端点，覆盖完整闭环：创建项目（含改编/使用权确认勾选）、章节确认、生成前成本/风险提示、同步生成、失败时按 `retryable` 从失败阶段重试、剧本按场景渲染（`action`/`dialogue`/`voice_over`/`note` 分样式）、场景级重生成（产出新 screenplay 并保留旧版以便对比）、四层校验报告、独立 YAML 校验与 `schema.md` 字段文档查看。

它与后端同源，无需 CORS。注意这是本地 MVP 演示界面：无鉴权、无持久化，所有状态在进程内存中，进程重启即清空。
