    ...
    <div class="grid2">
      ...
      <div class="col">
        <article class="panel">
          ...
          <div class="row">
            <div class="field"><label>标题</label><input type="text" v-model="form.title" /></div>
            <div class="field"><label>原作者</label><input type="text" v-model="form.original_author" /></div>
            <div class="field"><label>语言</label><input type="text" v-model="form.language" /></div>
          </div>
          <label>章节(至少 3 章才能生成)</label>
          <div v-for="(c, i) in form.chapters" :key="i" class="chapter-row">
            <input type="text" v-model="c.title" placeholder="章节标题" />
            <textarea v-model="c.text" placeholder="章节正文(作为不可信数据传入)"></textarea>
            <button class="ghost small del" @click="removeChapter(i)">删除</button>
          </div>
          <div class="btn-row">
            <button class="ghost small inline" @click="addChapter">+ 添加章节</button>
          </div>
          <div class="checkbox">
            <input id="rights" type="checkbox" v-model="form.rights_confirmed" />
            <label for="rights">我已确认拥有该文本的改编/使用权</label>
          </div>
          <button class="inline" :disabled="busy.create" @click="createProject">创建项目</button>
          <div v-if="project" class="kv" style="margin-top:14px">
        ...
        <article class="panel" v-if="project">
          ...
          <div v-if="chapters">
            ...
            <p class="small-note">列表只显示字数,不返回原文(契约约束)。</p>
          </div>
          <div class="btn-row"><button class="inline" :disabled="!canConfirm || busy.confirm" @click="confirmChapters">确认章节</button></div>
        </article>
        <article class="panel" v-if="notice">
          ...
          <p class="small-note">{{ notice.cost_notice }}</p>
          <div v-for="(r, i) in notice.risk_notice" :key="i" class="report-line muted" style="font-size:12px">· {{ r }}</div>
        </article>
        <article class="panel" v-if="canGenerate">
          <h2><span class="step">4</span> 生成剧本</h2>
          <div class="row gen2">
            <div class="field"><label>model</label><input type="text" v-model="gen.model" /></div>
            <div class="field"><label>output_language</label><input type="text" v-model="gen.output_language" placeholder="可空" /></div>
          </div>
          <div class="row gen2">
            <div class="field"><label>target_medium</label><input type="text" v-model="gen.target_medium" placeholder="可空" /></div>
            <div class="field"><label>adaptation_degree</label><input type="text" v-model="gen.adaptation_degree" placeholder="可空" /></div>
          </div>
          <div class="btn-row"><button class="inline" :disabled="busy.generate" @click="generate">生成</button></div>
          <div v-if="jobFailed" class="fail-box">
            ...
            <div class="btn-row">
              <button class="danger inline" :disabled="!canRetry || busy.retry" @click="retryJob">重试(从失败阶段恢复)</button>
      ...
      <!-- 右列 -->
      <div class="col">
        <article class="panel" v-if="history.length">
          ...
          <p class="small-note">场景重生成会产出新剧本,旧版本保留以便对比。</p>
        </article>
        <article class="panel" v-if="report">
          <h2>校验报告</h2>
          ...
          <div v-if="report.errors.length" style="margin-top:10px">
          ...
          <div v-if="report.warnings.length" style="margin-top:10px">
          ...
          <div v-if="report.suggested_fixes.length" style="margin-top:10px">
            <div v-for="(f, i) in report.suggested_fixes" :key="'f'+i" class="report-line muted">💡 {{ f }}</div>
          </div>
          <p v-if="reportEmpty" class="small-note">无错误、无警告。</p>
        </article>
        <article class="panel" v-if="scenes.length">
          <h2>剧本场景</h2>
        ...
        <article class="panel" v-if="artifacts">
          <h2>导出 YAML</h2>
        ...
        <article class="panel">
          ...
          <p class="small-note" style="margin-top:0">粘贴作者编辑后的 YAML,运行四层校验。该接口不存储传入内容。</p>
          <textarea class="mono" rows="8" v-model="yamlInput" placeholder="metadata: ..."></textarea>
          <div class="btn-row"><button class="inline" :disabled="busy.validate" @click="validateYaml">校验</button></div>
          <div v-if="yamlReport" style="margin-top:12px">
            ...
            <div v-for="(e, i) in yamlReport.errors" :key="'ye'+i" class="report-line" style="margin-top:6px">
              <span class="badge err">err</span> <code>{{ e.code }}</code> {{ e.message }}
              <span v-if="e.path" class="path">@ {{ e.path }}</span>
            </div>
        ...
        <article class="panel" v-if="activeScreenplayId">
