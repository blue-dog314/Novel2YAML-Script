...
- 演示视频：[百度网盘演示](https://pan.baidu.com/s/1haPiDoeprMYzso8x9aM3cg?pwd=2ntm) 提取码: 2ntm
...
系统**默认接入真实 DeepSeek 模型**(`deepseek-v4-flash`)。启动 API 前需配置 API key:

```bash
export DEEPSEEK_API_KEY=<你的 key>          # 必填:启用真实模型
export NOVEL_TO_SCREENPLAY_DEFAULT_MODEL=deepseek-v4-pro   # 可选:覆盖默认模型
uv run uvicorn api.app:app --reload
```

工作台「生成设置」可在 `deepseek-v4-flash` / `deepseek-v4-pro` 之间切换。可选环境变量:`DEEPSEEK_BASE_URL`、`DEEPSEEK_TIMEOUT_SECONDS`、`DEEPSEEK_MAX_RETRIES`。

真实 LLM 客户端基于 Python 标准库 `urllib` 实现,**不引入额外第三方依赖**;同样支持任意 OpenAI 兼容服务(通过 `OPENAI_API_KEY` / `OPENAI_MODEL` 等)。

离线、测试或无 key 时,显式传入模型名 `fake-model` 即回退到内置的确定性 `FakeLLMClient`(全量测试即依赖它保持可复现、不联网)。
...
- **分阶段生成流水线(`packages/generation`)**:章节摘要 → 关键事件 → 分场规划 → 场景内容块 → 后端装配,每一阶段产出受约束的结构化中间产物,而非「整本小说一次成稿」。失败处理遵循「同阶段至多自动修复一次、保留已完成中间产物、绝不导出非法 YAML」。默认接入真实 DeepSeek 模型,亦支持任意 OpenAI 兼容服务;离线测试可显式回退到确定性 `FakeLLMClient`。
